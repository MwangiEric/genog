    ...
    (
      <div style={{
        ...
        <div style={{ position: 'absolute', top: 40, display: 'flex', width: '100%', justifyContent: 'center' }}>
          <img src="https://ik.imagekit.io/ericmwangi/tklogo.png" width={variant === 'status' ? 320 : 220} height={80} style={{ objectFit: 'contain' }} />
        </div>
        ...
        <div style={{ 
          ...
          <div style={{ position: 'absolute', width: 800, height: 800, background: `radial-gradient(circle, ${COLORS.glow} 0%, transparent 70%)`, borderRadius: '50%' }} />
          {imageUrl && <img src={imageUrl} style={{ width: '90%', height: '90%', objectFit: 'contain', zIndex: 10 }} />}
        </div>
        ...
        <div style={{ 
          ...
          <div style={{ height: 5, width: 100, background: isLight ? COLORS.gold : COLORS.maroon, marginTop: 15 }} />

          {/* SPECS (Hidden in Banner to keep it clean) */}
          {variant !== 'banner' && (
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 10, marginTop: 30 }}>
              {[ { l: 'RAM', v: ram }, { l: 'ROM', v: rom }, { l: 'BAT', v: bat }, { l: 'SCR', v: scr } ].map(s => (
                <div key={s.l} style={{ background: COLORS.card, padding: '12px 18px', borderRadius: 15, border: `1px solid ${COLORS.border}`, display: 'flex', flexDirection: 'column', minWidth: '110px' }}>
                  <span style={{ color: COLORS.gold, fontSize: 12, fontWeight: 800 }}>{s.l}</span>
                  <span style={{ fontSize: 18, fontWeight: 900, color: isLight ? COLORS.maroon : '#FFF' }}>{s.v}</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ 
            ...
            boxShadow: `0 15px 30px ${isLight ? 'rgba(128,0,0,0.2)' : 'rgba(62, 180, 137, 0.2)'}`
          }}>
            <span style={{ fontSize: 60, fontWeight: 900, color: isLight ? '#FFF' : '#000' }}>KES {price}</span>
          </div>

          {/* FOOTER (Contact Info) */}
          <div style={{ 
            display: 'flex', width: variant === 'banner' ? '100%' : '90%', 
            ...
            justifyContent: 'space-between', alignItems: 'center', marginTop: 40 
             ...
             <div style={{ display: 'flex', alignItems: 'center' }}>
                <img src="https://ik.imagekit.io/ericmwangi/whatsapp.png" width={50} height={50} style={{ marginRight: 15 }} />
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                  <span style={{ fontSize: 28, fontWeight: 900, color: isLight ? COLORS.maroon : '#FFF' }}>0733 565861</span>
                  <span style={{ fontSize: 18, fontWeight: 800, opacity: 0.6 }}>0715 130013</span>
                </div>
             </div>
             <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                <span style={{ color: COLORS.gold, fontSize: 12, fontWeight: 800 }}>LOCATION</span>
                <span style={{ fontSize: 18, fontWeight: 900, color: isLight ? COLORS.maroon : '#FFF' }}>CBD OPP. MKU</span>
             </div>
          </div>
        </div>
